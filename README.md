...
- **MVP 做了什么**：用一个可交互 Demo 模拟 AI 如何根据门店信息生成套餐诊断、内容样片、核销提升建议、复盘表和下一步决策卡。
...
Demo 是一个可交互模拟原型。店主可以直接填写门店类型、商圈、主推套餐、当前困惑和近 7 天数据，再点击“生成诊断”获得一套可执行方案。顶部 3 个场景仍可一键填充示例：
...
当前版本先用规则和模板模拟完整工作流，不暴露密钥、不接后端。后续如果继续升级，可以把 `generateWithAI(input)` 接到真实大模型服务。
